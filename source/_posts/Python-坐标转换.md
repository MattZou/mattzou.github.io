...
updated: 
author: MattZou
index_img: https://mattblog.oss-cn-beijing.aliyuncs.com/img/index_img/wgs84.jfif/bg
categories: 软件使用
tags:
- 坐标转换
- Python
description: 国内地图API坐标系统转换
---

## 常用坐标系统
- WGS84 ：地理坐标系统，Google Earth和中国外的Google Map使用，另外，目前基本上所有定位空间位置的设备都使用这种坐标系统，例如手机的GPS系统。
- GCJ-02：投影坐标系统，也就是我们平常所说的火星坐标系，Google Map中国、高德和腾讯使用，这个是中国自己在WGS84基础上加密而成。
...
results = pd.read_csv(r'D:\kk.csv', encoding = 'gbk')

results['result'] = results.apply(gcj02towgs84 , 
      axis = 1, args = ('x','y')) 

results.to_csv('ss.csv', encoding = 'gbk')
```
